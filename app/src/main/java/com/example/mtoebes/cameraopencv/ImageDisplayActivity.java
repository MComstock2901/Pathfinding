...
public class ImageDisplayActivity extends Activity implements OnItemSelectedListener {
    ...
    private static final String tag_default = "default", tag_red = "red", tag_green = "green", tag_blue = "blue",
            ...
            tag_laplacian = "laplacian", tag_gaussian = "gaussian", tag_canny = "canny", tag_hough = "hough", tag_customColors = "CustomColors";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mImage = (ImageView) this.findViewById(R.id.surface_view);
    ...
    public Mat getMat(String tag) {
        ...
        if( mMats.containsKey(tag)){
            resMat = mMats.get(tag);
        }else{
        // Else create it
        switch(tag) {
            case tag_customColors:
                resMat = MatFilter.customColors(getMat(tag_default));
                break;
            case tag_hough:
                resMat = MatFilter.getHoughMat(getMat(tag_canny));
                break;
            case tag_canny:
                resMat = MatFilter.getCanny(getMat(tag_gaussian));
                break;
            case tag_gaussian:
                resMat = MatFilter.getGaussianBlur(getMat(tag_gray));
                break;
            case tag_laplacian:
                resMat = MatFilter.getLaplacian(getMat(tag_gaussian));
                break;
            case tag_sobel:
                resMat = MatFilter.getSobel(getMat(tag_gaussian), 1, 1);
                break;
            case tag_sobel_x:
                resMat = MatFilter.getSobel(getMat(tag_gaussian), 1, 0);
                break;
            case tag_sobel_y:
                resMat = MatFilter.getSobel(getMat(tag_gaussian), 0, 1);
                break;
            case tag_red:
                resMat = MatFilter.getChannel(mSrcMat, 0);
                break;
            case tag_green:
                resMat = MatFilter.getChannel(mSrcMat, 1);
                break;
            case tag_blue:
                resMat = MatFilter.getChannel(mSrcMat, 2);
                break;
            case tag_gray:
                resMat = MatFilter.getGrayScale(mSrcMat);
                break;
            default:
                resMat = mSrcMat;
                break;
